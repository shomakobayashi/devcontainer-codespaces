...
**対処法**：`gh extension install github/gh-copilot` の代わりに、npm 公式パッケージ（`github/copilot-cli` リポジトリの README に記載の正規手順）でインストールします。
...
なお、`copilot` と入力して `Y` を押すと VS Code の Copilot Chat 拡張が独自パスにバイナリを配置しますが、コンテナを Rebuild するたびに消えてしまうため、`npm install -g` で自動化するのが確実です。
...
マウントしても下記のエラーが出る場合は、トークンが無効になっているか `copilot` スコープが不足しています。
...
1. `gh auth status` で認証状態とスコープを確認する
2. `copilot` スコープが不足している場合は `gh auth refresh -s copilot` でスコープを追加する
